...
# 
# A dictionary containing various details about the slip system.
# For now using a simple orthorhombic unit cell
# with only one atom per unit cell

simple_orthorhombic = dict(
        latt_params=(2, 2, 2),
...
def calculate_disloc_energy_components(disloc_params,
                                       simulation_params):
    ...
    # c is the repeat distance paralell to line length
    c = slip_system['latt_params'][2] 
    
    # If we have information about basis systems 
    # and need to tranform the stiffness tensor:
    if 'stiffness_basis' in slip_system:
        elastic_tensor = transform_tensor(
                 slip_system['elastic_tensor'],
    ...
    # from the dislocation parameters
    (final_atoms,
    initial_atoms) = create_atoms(
    ...
    x_bonds, y_bonds = identify_bonds(final_atoms,
                                      initial_atoms,
                                      simulation_params)
    adjacent_y_bonds = identify_adjacent_y_bonds(final_atoms,
                                                 initial_atoms,
                                                 y_bonds,
                                                 simulation_params)
    # define lists of ideal bonds (i.e. unstrained at least locally)
    # these are unit vectors in the ideal direction.
    ideal_x_bonds = find_ideal_x_bonds(x_bonds, 
                                       initial_atoms, 
                                       simulation_params)
    ...
    (misaligned_bonds, 
     local_slip_plane_orientation) = identify_misaligned_bonds(
                     initial_atoms,
                     final_atoms,
                     x_bonds,
                     simulation_params
    ...
    # mask the positive x extreme, i.e. cells for
    # which two y_bonds do not exist.
    mask = einsum('ni,ni->n', x_bonds, x_bonds) == 0
    ...
    mask = np.logical_and(mask, 
                          np.fabs(initial_atoms[:,1]) > core_size)
    # calculate energy from them:
    strain_energy = calc_cumulative_strain_energy(strains,
                           initial_atoms,
                           slip_system=slip_system
                           )
    #the strain energy in bonds that aren't across the slip plane.
    misalignment_energy = calculate_misalignment_energy(
                                   misaligned_bonds,
                                   local_slip_plane_orientation,
                                   initial_atoms,
                                   slip_system=slip_system
                                   )
    slip_plane_strain_energy = calc_strain_energy_across_slip_plane(
                                   misaligned_bonds,
                                   local_slip_plane_orientation,
                                   initial_atoms,
                                   slip_system=slip_system)
...
def calculate_disloc_energy(disloc_params, simulation_params):
    energy = calculate_disloc_energy_components(
                      disloc_params,
                      simulation_params
                      )[0]
    return energy
    
def minimisable_energy(
            disloc_params,
            simulation_params,
            ref_disloc_params=np.array([1.5, 0.4, 0.4, -0.1])
            ):
    ref_energy = 2e-8
    energy = calculate_disloc_energy(
                  disloc_params=disloc_params,
                  simulation_params=simulation_params
                  )
...
def calc_strains(x_bonds,
    ...
    # the component of the x_bond along the ideal x direction 
    # as a fraction of b
    relative_x_lengths = einsum('ni,ni->n',
                                x_bonds,
                                ideal_x_bonds) / b
    mask = relative_x_lengths == 0
    relative_x_lengths[mask] = 1
    # turn to strain (note the correction of sign)
    del_ux_by_del_x = (relative_x_lengths 
                       - np.copysign(1, relative_x_lengths))
    strains[:,0] = del_ux_by_del_x                                                         
    # del_vy_by_del_y
    # similarly in y
    relative_y_lengths = einsum('ni,ni->n',
                                y_bonds,
                                ideal_y_bonds) / d
    mask = relative_y_lengths == 0
    relative_y_lengths[mask] = 1    
    del_v_by_del_y  = (relative_y_lengths 
                       - np.copysign(1, relative_y_lengths))
...
def create_atoms(
           disloc_params=np.array([1.2, 0.2, 0.2, -0.1],
           dtype=myfloat),
    ...
    if 'core_offset' in simulation_params[0]:
        (final_atoms,
         initial_atoms) = build.dislocation_long_output(
                    shape=shape,
                    alpha=alpha,
...
# ## Transformation of Basis Set
# 
# The elastic tensor is relative to the crystal axes 
# but the strains and stresses are relative to the burgers vector, 
# the slip plane normal and the line vector.
# Need to transform one or the other to match.
...
def transform_tensor(elastic_tensor, 
                     slip_system=simple_orthorhombic):
    ...
    new_C_ijkl = einsum('pi,qj,rk,sl,pqrs->ijkl',
                        transform,
                        transform,
                        transform,
                        transform,
...
def _old_find_ideal_x_bonds(x_bonds,
                            initial_atoms, 
                            slip_system=simple_orthorhombic):
    ...
    # a roll of +1 will align the ith bond with the position 
    # of its (i + 1)th neighbour,
    # which is in the positive x direction. 
    # Correct for atoms with the condition
    # x0 > 0 (NB not x0 == 0, which we assign to be 
    # ideally parallel to the x axis)
    rotated_bonds = np.roll(x_bonds, 1, axis=0)
    mask = x0 > 0

    ideal_x_bonds[mask] = rotated_bonds[mask]

    # a roll of -1 brings each bond on to the row at i-1,
    # corresponding to it's neighbour
    # in the -ve x direction,
    # which is required for x0 < -b (again not <=)
    rotated_bonds = np.roll(x_bonds, -1, axis=0)
    mask = x0 < -b
    ideal_x_bonds[mask] = rotated_bonds[mask]

    
    magnitudes = einsum('ni,ni->n',
                        ideal_x_bonds,
                        ideal_x_bonds) ** 0.5
    mask = magnitudes != 0
    ideal_x_bonds[mask] = (ideal_x_bonds[mask] 
                           / magnitudes[:,None][mask])
...
def find_y_bonds(initial_atoms, final_atoms, simulation_params):
    ...
    # we know some atoms will not have a 
    # neighbour in the y direcion:
    # missing at the top
    ...
    if motif_length == 1:
        # above the slip plane this rotation of
        # the array rows gets the right alignemnt
        roll = -2*x_range*motif_length
        mask = mask_bonded_in_y(initial_atoms,
                                final_atoms,
                                simulation_params,
                                roll)
        rotated_initial_positions = np.roll(initial_positions,
                                            roll,
                                            axis=0)
        rotated_final_positions = np.roll(final_positions,
                                          roll,
                                          axis=0)
        y_bonds[mask] = (rotated_final_positions[mask] 
                         - final_positions[mask])
        # below the slip lane this rotation gets the 
        # right alignment
        ...
        # if there is a motif longer than one it's 
        # likely to be two layers
        ...
        # above and below slip planes having 
    ...
    # number of hanging bonds at positive y extreme and 
    # across the slip plane
    # test to see if we've got it right
    # if not brute force the search
    y_bond_count = (np.count_nonzero(einsum('ni,ni->n',
                                    y_bonds,
                                    y_bonds)) 
                    + no_of_missing_bonds)
    if Y_bond_count != len(y_bonds):
        print('Brute forcing the y_bond search')
        # A slow backup if we don't find any bonds assume 
        # we've messed up:
    ...
    y_bond_count = (np.count_nonzero(einsum('ni,ni->n',
                    ...
                    + no_of_missing_bonds)
    if y_bond_count != len(y_bonds):
        print('Number of identified y bonds '
              + 'does not match the expected number')
    
    return y_bonds

def mask_bonded_in_y(initial_atoms, 
                     final_atoms,
                     simulation_params,
                     roll):
    ...
    if motif_length == 1:
        mask = (rotated_initial_positions 
                - initial_positions 
                - np.array([0, d, 0]))
        mask = einsum('ni,ni->n', mask, mask)
        mask = mask < 1e-16
        ...
        for d_i in slip_system['layer_spacings']:
            separations_i = (rotated_initial_positions 
                             - initial_positions
                             - np.array([0, d_i, 0]))
            separations_i = einsum('ni,ni->n', 
                                   separations_i,
                                   separations_i)
...
def identify_bonds(final_atoms, initial_atoms, simulation_params):
    """Create a list of bonds that correspond to the atoms
    ...
    n = 0
    # test
    x_bonds = find_x_bonds(initial_atoms, 
                           final_atoms,
                           simulation_params)
    y_bonds = find_y_bonds(initial_atoms,
                           final_atoms,
                           simulation_params)
...
def identify_adjacent_y_bonds(final_atoms, 
                              initial_atoms,
                              y_bonds,
                              simulation_params):
    ...
    rolled_final_atoms = np.roll(final_atoms,
                                 -motif_length,
                                 axis=0)
    rolled_init_atoms = np.roll(initial_atoms,
                                -motif_length,
                                axis=0)
    ...
    if test:
        ...
        adjacent_y_bonds = np.zeros((len(initial_atoms), 3),
                                    dtype=myfloat)
        for n in range(len(initial_atoms)):
            rolled_init_atoms = np.roll(initial_atoms, n, axis=0)
            rolled_y_bonds = np.roll(y_bonds, n, axis=0)
            # find difference between the offsets and 
            # the expected bonds:
            diffs = ((rolled_init_atoms - initial_atoms) 
                     - [b, 0, 0, 0])
    ...
    test = np.count_nonzero(mask) == expected_non_adjacent
    if not test:
        print('Something has gone wrong with' 
              + ' finding the adjacent y-bonds')
...
def find_ideal_x_bonds(x_bonds, initial_atoms, simulation_params):
    ...
    atoms_rolled_down = np.roll(initial_atoms, 
                                -motif_length,
                                axis=0)

    # Check that we have in fact found neighbours 
    # on wither side# if this is false then the 
    # atom is at an extreme edge of the simulation 
    # and does not have a neighbouring bond on both sides
    ...
    disps_down = (einsum('ni,ni->n',
                         disps_down,
                         disps_down) ** 0.5) - b
    mask = np.logical_and(mask, disps_up < 1e-8)
    mask = np.logical_and(mask, disps_down < 1e-8)

    # check that bonds exist either side of the 
    # atom/bond in question
    up_bond_lengths = einsum('ni,ni->n',
                             bonds_rolled_up,
                             bonds_rolled_up) ** 0.5
    down_bond_lengths = einsum('ni,ni->n',
                               bonds_rolled_down,
                               bonds_rolled_down) ** 0.5
    mask = np.logical_and(mask, up_bond_lengths != 0)
    mask = np.logical_and(mask, down_bond_lengths != 0)

    ideal_x_bonds[mask] = (bonds_rolled_down[mask]
                           + bonds_rolled_up[mask]) / 2

    with np.errstate(divide='ignore', invalid='ignore'):
        ideal_x_bonds /= (einsum('ni,ni->n',
                                 ideal_x_bonds,
                                 ideal_x_bonds) ** 0.5)[:,None]
        ideal_x_bonds = np.nan_to_num(ideal_x_bonds)

    if size[2] == 0:
        # there are 2 * n_y rows each having 3 bonds
        # that will be disqualified
        expected_no_of_falses = 6 * size[1] * motif_length
    else:
        expected_no_of_falses = (6 * size[1]
                                 * size[2]
                                 * motif_length)
    # Fall back on a slower method if it hasn't worked
    if len(x_bonds) != (np.count_nonzero(mask) 
                        + expected_no_of_falses):
        ...
        for i in range(len(x_bonds)):
            ...
            for j in range(len(x_bonds)):
                ...
                # on wither side# if this is false then the 
                # atom is at an extreme edge of the simulation 
                # and does not have a neighbouring bond on 
                # both sides
                mask = initial_atoms[:,1] == atoms_rolled_up[:,1]
                mask = np.logical_and(mask, 
                                      (initial_atoms[:,1]
                ...
                # check that bonds exist either side of 
                # the atom/bond in question
                up_bond_lengths = einsum('ni,ni->n',
                                         bonds_rolled_up,
                                         bonds_rolled_up) ** 0.5
                down_bond_lengths = (einsum('ni,ni->n',#
        ...
        x_bond_count = (np.count_nonzero(einsum('ni,ni->n',
                                               ideal_x_bonds, 
                                               ideal_x_bonds))
                        + expected_no_of_falses)
        if x_bond_count != len(x_bonds):
            print("Couldn't identify ideal bonds successfully")       
    return ideal_x_bonds

def identify_misaligned_bonds(initial_atoms,
                              final_atoms,
                              x_bonds,
                              simulation_params):
    ...
    # y0_A is the distance to the first layer of atoms above 
    # the slip plane:
    ...
    init_forward_mis_bonds = (init_positions[
                                    np.fabs(y0 - y0_A) < 1e-8
                                    ][1:] 
    - init_positions[np.fabs(y0 - y0_B) < 1e-8])
    test = ((init_forward_mis_bonds[:,0] > 0).all()
            and (init_forward_mis_bonds[:,0] <= b).all())
    test = test and ((np.fabs(init_forward_mis_bonds[:,1]) 
                     - (y0_A - y0_B)) < 1e-8).all

    if test:
        forward_misaligned_bonds = (
               final_positions[np.fabs(y0 - y0_A) < 1e-10][1:] 
               - final_positions[np.fabs(y0 - y0_B) < 1e-8]
                                    )
    else:
        print('Could not identify forward'
              + ' leaning misaligned bonds!')

    init_backward_mis_bonds = (init_positions[
                                      np.fabs(y0 - y0_A) < 1e-8
                                              ][:-1] 
                               - init_positions[
                                      np.fabs(y0 - y0_B) < 1e-8
                                                ]
    test = (init_backward_mis_bonds[:,0] <= 0).all() 
            and (init_backward_mis_bonds[:,0] > -b).all()
    test = test and (init_backward_mis_bonds[:,1] == d).all
    if test:
        backward_misaligned_bonds = (
               final_positions[np.fabs(y0-y0_A)<1e-8][:-1]
               - final_positions[np.fabs(y0 - y0_B) < 1e-8]
                                     )
    else:
        print('Could not identify backward leaning '
        + 'misaligned bonds!')

    misaligned_bonds = np.vstack((forward_misaligned_bonds,
                                  backward_misaligned_bonds))
    ...
    # mask_1 is the positions of those atoms that are 
    # immediately below the slip plane
    # from amongst all of the atoms

    mask_2 = (np.fabs((initial_atoms - rolled_atoms)[:,0] - b)
              < 1e-8)
    # those atoms separated in x by a burgers vector
    # next check that all the bonds are real
    mask_2 = np.logical_and(
               mask_2, 
               einsum('ni,ni->n', x_bonds, x_bonds) != 0
                            )
    mask_2 = np.logical_and(
          mask_2,
          einsum('ni,ni->n', rolled_x_bonds, rolled_x_bonds) != 0
                            )

    # mask two is the position of those initial_atoms and 
    # rolled_atoms that are immediate neighbours
    # from amongst the complete set and that the 
    # corresponding bonds exist

    # use mask_2 to find local plane orientations across 
    # the whole crystal:
    slip_plane_orientation[mask_2] = ((x_bonds[mask_2]
                                       + rolled_x_bonds[mask_2]) 
                                       / 2)
    # use mask_1 to cut this down to just the atoms
    # immediately below the slip plane slip plane
    slip_plane_orientation = slip_plane_orientation[mask_1]
    # this will leave zeros
    slip_plane_orientation = np.vstack((slip_plane_orientation,
                                        slip_plane_orientation))
    np.count_nonzero(einsum('ni,ni->n', 
                            slip_plane_orientation,
                            slip_plane_orientation))
    ...
    test = (np.count_nonzero(einsum('ni,ni->n',
                                   slip_plane_orientation,
                                   slip_plane_orientation)) 
            == len(slip_plane_orientation) - 4)
    if not test:
        print('Couldn't identify the slip plane orientation,'
              + ' you'll have to write a brute force route.')
    ...
    with np.errstate(divide='ignore', invalid='ignore'):
        slip_plane_orientation /= (
                np.einsum('ni,ni->n',
                          slip_plane_orientation, 
                          slip_plane_orientation) ** 0.5)[:,None]
        slip_plane_orientation = (
                np.nan_to_num(slip_plane_orientation)
                                  )  
    return misaligned_bonds, slip_plane_orientation   
 
 

def frenkel_approx(phi,
                   initial_atoms, 
                   slip_system=simple_orthorhombic):
    b = slip_system['latt_params'][0]
    d = slip_system['latt_params'][1]
    ...
    if 'stiffness_basis' in slip_system:
        transform = find_transform(
                           slip_system['stiffness_basis'],
                           slip_system['dislocation_basis']
                                   )
        elastic_tensor = transform_elastic_tensor(
                            elastic_tensor, 
                            slip_system=slip_system
                                                  )
    if 'layer_stiffnesses' not in slip_system:
        prefactor = (elastic_tensor[5,5] 
                     * b
                     * d
                     * l 
                     / (4 * (np.pi**2)))
        ...
        if 'stiffness_basis' in slip_system:
            ...
            elastic_tensor_0 = transform_elastic_tensor(
                              elastic_tensor_0, 
                              slip_system=slip_system
                                                        )
            elastic_tensor_1 = transform_elastic_tensor(
                              elastic_tensor_1, 
                              slip_system=slip_system
                                                        )
        ...
        y0 = initial_atoms[:,1]
        # the position of the first layer of atoms below the 
        # slip plane:
        ...
        prefactor_0 = (elastic_tensor_0[5,5] 
                       * b 
                       * d_0 
                       * l 
                       / (4 * (np.pi**2)))
        periodic_term_0 = 1 - np.cos(2 * np.pi * phi[mask_0] / b) 
        energy_0 = np.sum(units 
                          * prefactor_0
                          * periodic_term_0) / 2 
        
        prefactor_1 = (elastic_tensor_1[5,5]
                       * b 
                       * d_1 
                       * l 
                       / (4 * (np.pi**2)))
        periodic_term_1 = 1 - np.cos(2 * np.pi * phi[mask_1] / b) 
        energy_1 = np.sum(units
                          * prefactor_1
                          * periodic_term_1) / 2 
...
def calculate_misalignment_energy(misaligned_bonds, 
    ...
    # frenkel approximation includes the factor of 0.5 to
    # avoid double counting
    misalignment_energy += frenkel_approx(phi, 
                                          initial_atoms,
                                          slip_system=slip_system)
    return misalignment_energy

def calc_strain_energy_across_slip_plane(
               misaligned_bonds,
               local_slip_plane_orientation,
               initial_atoms,
               slip_system=simple_orthorhombic):
    ...
    if 'layered_stiffnesses' in slip_system:
        ...
        local_slip_plane_normal = (
               np.zeros_like(local_slip_plane_orientation,
                             dtype=myfloat)
                                   )
        local_slip_plane_normal[:,0] = (
                        - local_slip_plane_orientation[:,1] 
                                        )
        local_slip_plane_normal[:,1] = (
                        local_slip_plane_orientation[:,0]
                                        )
        delta_0 = (einsum('ni,ni->n',
                         misaligned_bonds[mask_0],
                         local_slip_plane_normal[mask_0])
                   - d0)
        delta_1 = (einsum('ni,ni->n',
                         misaligned_bonds[mask_1],
                         local_slip_plane_normal[mask_1])
                   - d1)
        ...
        strain_energy_0 = 0.5 * einsum('ij,nj,ni->',
                                       elastic_tensor_0,
                                       strains_0,
                                       strains_0)
        strain_energy_1 = 0.5 * einsum('ij,nj,ni->', 
                                       elastic_tensor_1,
                                       strains_1,
                                       strains_0)
        ...
        elastic_tensor = slip_system['elastic_tensor']
        strain_energy = 0.5 * einsum('ij,nj,ni->',
                                     elastic_tensor,
                                     strains,
                                     strains)
    ...
    # over 2 because there's two bonds for each atom across 
    # the slip plane
    slip_plane_strain_energy = strain_energy / 2
    return slip_plane_strain_energy

def calc_cumulative_strain_energy(strains, 
                                  initial_atoms,
                                  slip_system=simple_orthorhombic):
    '''given a 2D array each row of which is a strain in voigt 
       notation and a slip system with details of a stiffness 
       tensor (6x6) in GPa and lattice parameters in Angstroms
    ...
    if 'stiffness_basis' in slip_system:
        transform =find_transform(slip_system['stiffness_basis'],
                                  slip_system['dislocation_basis'])
        elastic_tensor = transform_elastic_tensor(
                                      elastic_tensor,
                                      slip_system=slip_system
    ...
    # if we don't have different bond lengths and elastic moduli
    # in different layers:
    if 'layer_stiffnesses' not in slip_system:
        strain_energy = 0.5 * einsum('ij,nj,ni->',
                                     elastic_tensor,
