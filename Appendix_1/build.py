...
simple_orthorhombic = dict(latt_params=(3.2, 2.3, 3.2),
                           motif = np.array([0.,0.5,0.,0.]),
    ...
    for z in range(-z_range, z_range):
        for y in range(-y_range, y_range):
            for x in range(-x_range, x_range):
                ...
                start = n*len(system['motif'])
                stop = (n+1)*len(system['motif'])
                atoms[start:stop] = motif
                n += 1 
    # In analogy with the defective case we will 
    # build the model the same way principly this
    # will mean we include the negative extreme
    # but exclude the positive extreme

    y_cutoff = d * y_range
    z_cutoff = c * z_range
    x_cutoff = b * x_range
    # create a mask that returns true for those atoms 
    # inside the simulation:
    # y and z directions are easy, just hard boundaries 
    # at plus/minus the cutoff
    
    y_mask = np.logical_and(y_cutoff > atoms[:,1],
                            atoms[:,1] >= -y_cutoff)

    z_mask = np.logical_and(atoms[:,2] < z_cutoff,
                            atoms[:,2] >= -z_cutoff)
    # x direction depends on whether it is above 
    # the slip plane or below it +ve y cutoff is x_cutoff
    x_mask_above = np.logical_and(atoms[:,1] >= 0, 
                                  atoms[:,0] < x_cutoff)
    x_mask_above = np.logical_and(x_mask_above, 
                                  atoms[:,0] >= -x_cutoff)
    #below the slip plane, -ve y, the cutoff is less by b/4
    x_mask_below = np.logical_and(atoms[:,1] < 0,
                                  atoms[:,0] >= -(x_cutoff))
    x_mask_below = np.logical_and(x_mask_below,
                                  atoms[:,0] < (x_cutoff))
    x_mask = np.logical_or(x_mask_above, x_mask_below)
...
def dislocation_long_output(shape=[5, 5, 5], 
    ...
    # The width parameter only makes sense if it's positive:
    ...
    if shape[2] == 0: # It makes sense that for some simulations 
        # we only want one layer in the z direction, 
        # along which there is symmetry
        
        x_range = int(shape[0])
        y_range = int(shape[1]) # in units of unit cells
        z_range = 2
        
        init_atoms = create_2D_initial_atoms(x_range,
                                             y_range,
                                             motif,
                                             alpha,
                                             latt_params,
        ...
        init_atoms = create_3D_initial_atoms(x_range,
                                             y_range,
                                             z_range,
                                             motif,
                                             alpha,
    ...
    atoms = np.copy(init_atoms)
    atoms[:,0] += calc_x_disp(init_atoms, 
                              disloc_params,
                              latt_params)
    atoms[:,1] += calc_y_disp(init_atoms,
                              disloc_params,
                              latt_params)
    ...
    x_cutoff = b * x_range
    # create a mask that returns true for those atoms 
    # inside the simulation: y and z directions are easy,
    # just hard boundaries at plus/minus the cutoff
    y_mask = np.logical_and(y_cutoff > init_atoms[:,1],
                            init_atoms[:,1] >= -y_cutoff)

    z_mask = np.logical_and(init_atoms[:,2] < z_cutoff,
                            init_atoms[:,2] >= -z_cutoff)
    # x direction depends on whether 
    # it is above the slip plane or below it
    # +ve y cutoff is x_cutoff
    
    x_mask_above = np.logical_and(init_atoms[:,1] >= 0,
                                  init_atoms[:,0] < x_cutoff)
    x_mask_above = np.logical_and(x_mask_above,
                                  init_atoms[:,0] >= -x_cutoff)
    #below the slip plane, -ve y, the cutoff is less by b/4
    x_mask_below = np.logical_and(init_atoms[:,1] < 0, 
                                  init_atoms[:,0]>=-(x_cutoff-b/2))
    x_mask_below = np.logical_and(x_mask_below,
                                  init_atoms[:,0] < (x_cutoff-b/2))
    x_mask = np.logical_or(x_mask_above, x_mask_below)
    ...
    if net_charge != 0 and calc_type == 'electrostatic':
        print('Net charge is not zero!'
              + '(alpha = {:g})'.format(alpha))

    if len(atoms) != len(initial_atoms):
        print('Atoms and Initial Atoms do not have '
        + 'the same length, stuff has gone wrong...')
    assert len(atoms) == len(initial_atoms)
    
    return atoms, initial_atoms

def create_2D_initial_atoms(x_range,
                            y_range,
                            motif,
                            alpha,
                            latt_params,
    ...
    # The dimensions are extended by 2 to allow
    # control of the edge effects
    if motif.ndim == 2:
        motif_length = len(motif)
    elif motif.ndim == 1:
        motif_length = 1
    else:
        raise Exception("Motif has ndim > 2 and cannot be" 
                        + "interpreted as either an atom"
                        + "or a list of atoms") 
    ...
    grid[:,:2] = np.mgrid[-n_x:n_x, -n_y:n_y].T.reshape(-1,2)
    grid = grid.astype(myfloat)
    # we might need to shift everything to put the core
    # of the dislocation in the right place
    ...
    x0[y0 < 0] += 0.5
    x0 -= alpha
    # This is now a grid with one row for ach unit cell origin,
    # tile it up to add a row for each atom:
    if motif_length != 0:
...
def create_3D_initial_atoms(x_range,
                            y_range,
                            z_range,
                            motif,
                            alpha,
    ...
    # The dimensions are extended by 2 
    # to allow control of the edge effects
    if motif.ndim == 2:
        motif_length = len(motif)
    elif motif.ndim == 1:
        motif_length = 1
    ...
    grid = np.mgrid[-n_x:n_x, -n_y:n_y, -n_z:n_z].T.reshape(-1,3)
    grid = grid.astype(myfloat)
    # we might need to shift everything to put 
    # the core of the dislocation in the right place
    # within the unit cell:
    grid += core_offset
    x0 = grid[:,0]
    y0 = grid[:,1]
    ...
    x0[y0 < 0] += 0.5
    x0 -= alpha
    # This is now a grid with one row for each unit cell origin,
    # tile it up to add a row for each atom:
    if motif_length != 0:
        grid = np.repeat(grid, motif_length, axis=0)
...
def calc_x_arctan_term(x0, y0, w, b):
    ...
    arc_tan_term = (np.arctan(arc_tan_argument)
                    - (np.pi 
                       * 0.5
                       * np.copysign(1, x0) 
                       * np.copysign(1, y0)))
