...
slip_system = dict(
   ...
   elastic_tensor=np.array([[171.0, 114.9, 114.9, 0,     0,     0],
         ...
         dislocation_basis = [np.array([1,-1,0], 
                                       dtype=myfloat)/np.sqrt(2),
                              np.array([1,1,1],
                                       dtype=myfloat)/np.sqrt(3),
                              np.array([-1, -1, 2],
                                       dtype=myfloat)/np.sqrt(6)] # 
                   )

# alternatively use a random slip system from the inputs module.
# inputs.systems[np.random.randint(len(inputs.systems))]
...
def callback(disloc_variables):
    ...
    #print(disloc_variables,
           '\n',
           ec.calculate_disloc_energy_components(disloc_variables, 
                                                 sim_params))
    return None
...
while time.time() - start < 40000: # i.e. run overnight
    ...
    try:
        ...
        if init_disloc_params.shape == ():
            init_disloc_params = np.reshape(init_disloc_params,
                                            (1,))
    except:
        init_disloc_params = np.array([1.98, 0.25, 0.38, -0.1],
                                      dtype=myfloat)
    ...
    init_disloc_params = (optimal_disloc_vars 
                          * (1 + (np.random.random() - 0.5) / 20))

    if optimal_disloc_vars.shape == ():
        optimal_disloc_vars = np.array([optimal_disloc_vars])
    np.savetxt(label+'_disloc_variables.csv',
               optimal_disloc_vars,
               delimiter=',')

    print('Optimum dislocation parameters: ', optimal_disloc_vars)
    (final_atoms, 
     initial_atoms) = ec.create_atoms(
               disloc_params=optimal_disloc_vars, 
               simulation_params=sim_params
                                      )
    energies = ec.calculate_disloc_energy_components(
                                    optimal_disloc_vars,
                                    sim_params
                                                     )
    print(energies)
