...
def calculate_ionic_energy(atoms, 
    ...
    # If the atoms contained in a temp_atoms.npy file match the 
    # tail end of the array of atoms we have exactly then the 
    # simulation must have had the same parameters at creation.
    # So we can skip the leading end of the array.
    ...
    if test:
        ...
        if np.array_equal(atoms_from_file,
                          atoms[-len(atoms_from_file):]):
    ...
    for n, ref_atom in enumerate(atoms):
        ...
        for m in range(0,remaining_atoms.shape[0],step_size):
            ...
            masked_seps_squared = seps_squared[(
                           separations < 320 * system['r0']
                                                )]
            # Using only those bonds less than 320 times larger than
            # the smallest one will place an upper bound on the 
            # computation required without affecting the results at 
            # 320 times the shortest bond the LJ term is below the 
            # machine precision of a 64 bit float.
            masked_seps_to_minus6 = np.power(masked_seps_squared,
                                             -3) # as scalars
            charge_products = charges * ref_charge 
            masked_charge_products = charge_products[(
                       separations < 320 * system['r0']
                                                      )]
        ...
        # If siginificant time has past then a checkpoint will be 
        # created. We have found the energy for each atom iterated 
        # over so far wrt each other and to the atoms in remaining 
        # atoms. Hence saving  the energies and the remaining atoms 
        # provides a suitable restart point
    ...
    return (total_energy,
            electrostatic_energy,
            repulsive_energy) # all in Joules

# The Lennard Jones Parameters are included 
# in the details of the system
def LJ6_energy(seps_to_minus6,
               charge_products,
               ref_charge,
               system=build.NaCl):
    ...
    ca_energy = (- system['B_ca'] 
                 * np.sum(seps_to_minus6[charge_products < 0]))
    energy += ca_energy

    if ref_charge > 0: #i.e. a cation
        # cation-cation interaction qq > 0
        # mask where qq < 0
        cc_energy = (- system['B_cc'] 
                     * np.sum(seps_to_minus6[charge_products>0]))
        energy += cc_energy
    ...
    if ref_charge < 0: #i.e. an anion
        ...
        # mask where qq < 0 
        aa_energy = (- system['B_cc']
                     * np.sum(seps_to_minus6[charge_products>0]))
        energy += aa_energy
        
    return energy #in joules

def LJ12_energy(seps_to_minus12,
                charge_products,
                ref_charge,
                system=build.NaCl):
    ...
    ca_energy = (system['A_ca']
                 * np.sum(seps_to_minus12[charge_products < 0]))
    energy += ca_energy

    if ref_charge > 0: #i.e. a cation
        # cation-cation interaction qq > 0
        # mask where qq < 0
        cc_energy = (system['A_cc']
                     * np.sum(seps_to_minus12[charge_products>0]))
        energy += cc_energy
...
def calc_lammps_coloumbic_energy(atoms,
                                 slip_system=build.NaCl_110_001):
...
def calc_lammps_energy(atoms, slip_system=build.NaCl_110_001):
    energy = calc_lammps_coloumbic_energy(atoms,
